...
class Generator(nn.Module):
    def __init__(self, num_blocks):
        ...
        self.upsample = nn.Sequential(
            nn.ConvTranspose2d(gen_channel*4, gen_channel*2, gen_kernel, stride=2, padding=1, output_padding=1), # 128x128x128
            nn.BatchNorm2d(gen_channel*2),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(gen_channel*2, gen_channel, gen_kernel, stride=2, padding=1, output_padding=1), # 256x256x64
            nn.BatchNorm2d(gen_channel),
            nn.ReLU(inplace=True)
        )
